...
def load_resources():
    ...
    try:
        ...
        for name, model in models.items():
            if len(model.input_shape) != expected_features[name] + 1:  # +1 for batch dim
                raise ValueError(
                    f"Model {name} expects {expected_features[name]} features, "
...
def prepare_features(gender, age, bmi, smoking, hypertension, heart_disease, glucose=0, hba1c=0):
    ...
    # Add glucose for Model 2
    if glucose > 0:
        features.append(float(glucose))
        
    # Add HbA1c for Model 3
    if hba1c > 0:
...
def main():
    ...
    if submitted:
        try:
            ...
            expected_features = 7 + int(glucose > 0) + int(hba1c > 0)
            ...
            **Risk Interpretation:**
            ...
            st.json({
                ...
                "model_expected": models[model_key].input_shape
            })
